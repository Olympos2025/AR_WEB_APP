...
const CalibrationPanel: React.FC<Props> = ({ options, onChange, heading, accuracy, t }) => {
  return (
    <div className="bg-slate-900 border border-slate-800 p-3 rounded space-y-2">
      ...
      <p className="text-sm text-slate-300">{t.accuracy}: {accuracy ? `±${accuracy.toFixed(1)}m` : 'N/A'}</p>
      <p className="text-sm text-slate-300">{t.heading}: {heading ? `${heading.toFixed(0)}°` : 'N/A'}</p>
